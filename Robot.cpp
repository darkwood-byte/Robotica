...
void Robot::Move(int pan_goal, int tilt_goal) {
    if(pan_motor.GetRange() == RANGE_NOT_DEFINED || tilt_motor.GetRange() == RANGE_NOT_DEFINED )return;
    ...
    int inverted_tilt = TILT_MAX_DEGREES - (tilt_clamped - TILT_MIN_DEGREES);
    tilt_motor.goal = (tilt_motor.GetRange() * (inverted_tilt - TILT_MIN_DEGREES)) / (TILT_MAX_DEGREES - TILT_MIN_DEGREES);
}
